...
class AdamSGD:
    '''
    AdamSGD strategy. Better than vanilla, but still not that good
    '''
    def __init__(self,model,loss,beta_m=1-1e-1,beta_v=1-1e-3,epsilon=1e-8,alpha=1e-2):
        ...
        self.vw=[ [[0 for i in range(self.model.nodes[l])] for j in range(self.model.nodes[l+1])]  for l in range(self.model.total_layers-1)]
        self.vb=[ [0 for j in range(self.model.nodes[l+1])]  for l in range(self.model.total_layers-1)]              
        ...
        for l in range(self.model.total_layers-1):
            for j in range(self.model.nodes[l+1]):
                for i in range(self.model.nodes[l]):
                    ...
                    dw*=self.mw[l][j][i]/(1-self.beta_m_ac)
                ...
                dw*=self.mb[l][j]/(1-self.beta_m_ac)
