...
class AdaMaxSGD(StochasticGradientDescent):
    ...
    def __init__(self,loss,beta_m=1-1e-1,beta_v=1-1e-3,epsilon=1e-8,alpha=1e-2):
        ...
        #The "bias corrected" m needs beta^iteration, so I need something like this
        ...
        self.vw_max=[ [[0 for i in range(self.Q.model.nodes[l])] for j in range(self.Q.model.nodes[l+1])]  for l in range(self.Q.model.total_layers-1)]
        self.vb_max=[ [0 for j in range(self.Q.model.nodes[l+1])]  for l in range(self.Q.model.total_layers-1)]              
        ...
        for l in range(self.Q.model.total_layers-1):
            for j in range(self.Q.model.nodes[l+1]):
                for i in range(self.Q.model.nodes[l]):
                    ...
                    self.vw_max[l][j][i]=np_max([self.beta_v*self.vw_max[l][j][i], np_abs(self.Q.dQdw) ]) 
                ...
                self.vb_max[l][j]=np_max([self.beta_v*self.vb_max[l][j], np_abs(self.Q.dQdb) ]) 
