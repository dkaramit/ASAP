...
class NAdam_SGD:public StochasticGradientDescent<LD,lossFunc>{
    public:
    ...
    NAdam_SGD(lossFunc *Q, LD beta_m=1-1e-1, LD beta_v=1-1e-3, LD epsilon=1e-8, LD alpha=1e-2):StochasticGradientDescent<LD,lossFunc>(Q){
        this->beta_m=beta_m;
        this->beta_v=beta_v;
        ...
        for(unsigned int l=0; l<this->Q->layers-1; ++l){
            for(unsigned int j=0; j< this->Q->model->nodes[l+1] ; ++j){
                for(unsigned int i=0; i< this->Q->model->nodes[l] ; ++i){
                    ...
                    dw*=(beta_m*mw[l][j][i] + (1-beta_m)*(this->Q->dQdw))/(1-beta_m_ac);
                ...
                dw*=(beta_m*mb[l][j] + (1-beta_m)*(this->Q->dQdb))/(1-beta_m_ac);
