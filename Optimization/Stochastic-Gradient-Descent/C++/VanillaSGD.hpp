...
class VanillaSGD{
    ...
    VanillaSGD(const lossFunc &Q, const vec2 &input_data, const vec2 &output_data, LD alpha=1e-3);
...
// Constructor
Vanilla_SGD_Template
Vanilla_SGD_Namespace::VanillaSGD(const lossFunc &Q, const vec2 &input_data, const vec2 & output_data, LD alpha){
    this->Q=Q;
...
LD Vanilla_SGD_Namespace::update(LD abs_tol, LD rel_tol){
    ...
    this->Q.Grad(this->input_data[index],
                this->output_data[index],
                this->grad);
