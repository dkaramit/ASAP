...
template<class diffeq, int number_of_eqs, class RK_method>
RK<diffeq, number_of_eqs, RK_method>:: RK(diffeq dydt, double  (&init_cond)[number_of_eqs] , int N){
        ...
        this->solution = new double*[number_of_eqs];
        ...
        this->k=new double*[number_of_eqs];
        for(int i = 0; i < number_of_eqs ;++i) {
                this->k[i] = new double[ this->method.s];
                } 
        
        this->ak=new double[number_of_eqs];
        this->bk=new double[number_of_eqs];
...
//The destructor
template<class diffeq, int number_of_eqs, class RK_method>
RK<diffeq, number_of_eqs, RK_method>::~RK(){
...
/*-----------------------Begin: sum_ak---------------------------------*/
template<class diffeq, int number_of_eqs, class RK_method>
void RK<diffeq, number_of_eqs, RK_method>::sum_ak(int stage){
    ...
    // for (int eq = 0; eq <number_of_eqs ; eq++){this->ak[eq]=0.;  }//again redundant but it is more clear this way
    for (int eq = 0; eq <number_of_eqs ; eq++){
...
/*-----------------------Begin: sum_bk---------------------------------*/
template<class diffeq, int number_of_eqs, class RK_method>
void RK<diffeq, number_of_eqs, RK_method>::sum_bk(){
    ...
    // for (int eq = 0; eq <number_of_eqs ; eq++){this->bk[eq]=0.;  }//again redundant but it is more clear this way
    for (int eq = 0; eq <number_of_eqs ; eq++){
...
/*---------------------------------------------------Begin: Get next step-------------------------------------------------------------------------------*/
template<class diffeq, int number_of_eqs, class RK_method>
void RK<diffeq, number_of_eqs, RK_method>::next_step(){
        if (this->current_step>= this->No_steps -1 )
        {
            ...
            for (int eq = 0; eq < number_of_eqs ; eq++){this->yn[eq]=this->solution[eq][this->current_step-1];}
            // then calculate f(n_n,t_n)
            this->dydt(this->fyn, this-> yn,this->tn );
            // with these we can calculate and fill \vec{k}_1
            for( int eq = 0; eq < number_of_eqs; eq++ ){ this->k[eq][0]=this->fyn[eq];  }
            ...
            for (int stage = 0; stage < (this->method).s; stage++){
                ...
                for (int eq = 0; eq < number_of_eqs ; eq++){this->yn[eq]=this->solution[eq][this->current_step-1]+ak[eq];}
                ...
                for( int eq = 0; eq < number_of_eqs; eq++ ){ this->k[eq][stage]=this->fyn[eq]; }
            }
...
template<class diffeq, int number_of_eqs, class RK_method>
void RK<diffeq, number_of_eqs, RK_method>::solve(){
